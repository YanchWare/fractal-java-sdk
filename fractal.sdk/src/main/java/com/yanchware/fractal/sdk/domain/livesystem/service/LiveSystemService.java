...
public class LiveSystemService extends Service {
  ...
  private static LiveSystemMutationDto getLiveSystemMutationResponse(
    String requestName,
    String bodyContents) throws ProviderException
  {
    try {
  ...
  private static LiveSystemComponentMutationDto getLiveSystemComponentMutationResponse(
    String requestName,
    String bodyContents) throws ProviderException
  {
    try {
    ...
    HttpRequest request) throws IOException, InterruptedException, InstantiatorException, ProviderException
  {
    ...
    if (liveSystemMutationResponse == null) {
      String message = String.format("The LiveSystem [%s] exists, but the LiveSystem's provider LiveSystem is still " +
          "empty or not fully updated for mutation [%s]",
    ...
    if (liveSystemMutationResponseComponents == null) {
      throw new ProviderException(String.format("Live System [%s] instantiation failed for mutation [%s] as component" +
          " list is null",
        liveSystemIdStr,
        liveSystemMutationId));
    }
    ...
    switch (liveSystemMutationResponse.status()) {
      ...
      case FAILED -> {
        ...
        if (StringUtils.isBlank(messageToThrow)) {
          log.warn("Live System [id: '{}'] instantiation failed but erorrMessage is not visible yet, retrying: {}",
            liveSystemId, getStatusFromComponents(liveSystemMutationResponseComponents));
          ...
          log.warn("Live System [id: '{}'] instantiation failed: {}", liveSystemId,
            getStatusFromComponents(liveSystemMutationResponseComponents));
          throw new ProviderException(messageToThrow);
        }
      }
      case CANCELLED -> {
        log.warn("Live System [id: '{}'] instantiation cancelled: {}", liveSystemId,
          getStatusFromComponents(liveSystemMutationResponseComponents));

        var cancelledInstantiationMessage = String.format(
          "Live System [%s] instantiation did not complete for mutation [%s] as the latter has been cancelled by an " +
            "operator",
          liveSystemId,
          liveSystemMutationId);
      ...
      default ->
        throw new ProviderException(String.format("Live System [%s] instantiation has an unknown state for mutation " +
            "[%s]: [%s]",
          liveSystemIdStr,
          liveSystemMutationId,
  ...
  private void logInstantiationCompleted(
    LiveSystemIdValue liveSystemId,
  ...
  private String getFailedComponentsMessageToThrow(
    String liveSystemId,
    String liveSystemMutationId,
    ...
    ComponentInstantiationException
  {
    ...
    switch (component.getStatus()) {
      ...
      default ->
        throw new ProviderException(String.format("Instantiation has an unknown state '%s' [LiveSystemId: '%s', " +
            "ComponentId: '%s', MutationId: '%s']",
          component.getStatus(),
          liveSystemId,
          componentId,
          mutationId));
    ...
    EnvironmentReference environmentReference) throws InstantiatorException
  {
    ...
    try {
      ...
      throw new InstantiatorException("Error processing InstantiateLiveSystemCommandRequest because of " +
        "JsonProcessing", e);
    }
