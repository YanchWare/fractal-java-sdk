...
public class BlueprintComponentDtoTest {
  ...
  public void blueprintComponentValid_when_azurePostgresLiveSystemComponentConverted() {
    ...
    Optional<PaaSPostgreSqlDbImpl> db1Optional = apg.getDatabases().stream().filter(x -> x.getId().getValue().equals(azurePgDbBlueprintCompDto.getId())).findFirst();
    ...
    Optional<PaaSPostgreSqlDbImpl> db2Optional = apg.getDatabases().stream().filter(x -> x.getId().getValue().equals(azurePgDbBlueprintCompDto2.getId())).findFirst();
