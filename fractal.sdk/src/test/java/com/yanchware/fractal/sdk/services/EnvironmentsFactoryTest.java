...
class EnvironmentsFactoryTest {
  ...
  private EnvironmentsFactory environmentsFactory;
  ...
  void setUp(WireMockRuntimeInfo wmRuntimeInfo) {    
    ...
    environmentsFactory = new EnvironmentsFactory(httpClient, sdkConfiguration, RetryRegistry.ofDefaults());
  ...
  void fetchEnvironment_should_ReturnEnvironmentIdDto() throws Exception {
    ...
    // When
    var response = environmentsFactory.fetchEnvironment(mockEnvironment);

    // Then
    verify(1, getRequestedFor(urlPattern));

    assertThat(response).isNotNull();
    assertThat(response.id().getType().toString()).isEqualTo(mockEnvironment.getEnvironmentType().toString());
  ...
  void createOrUpdateEnvironment_whenEnvironmentExists_shouldUpdateEnvironment() throws Exception {
    ...
    // When
    environmentsFactory.createOrUpdateEnvironment(mockEnvironment);
  ...
  void updateEnvironment_shouldExecutePutRequest() throws Exception {
    ...
    // When
    var response = environmentsFactory.updateEnvironment(mockEnvironment);
  ...
  void fetchCurrentInitialization_should_ReturnValidAzureInitializationRunResponse() throws Exception {
    ...
    // When
    var response = environmentsFactory.fetchCurrentAzureInitialization(mockEnvironment);
  ...
  void initializeSubscription_should_HandleInitializationInProgress() throws Exception {
    ...
    // When
    environmentsFactory.initializeAgents(mockEnvironment, Duration.ofSeconds(2));
