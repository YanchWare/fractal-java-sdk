...
public class LiveSystemKubernetesComponentDtoTest {
  protected void assertCaaSComponents(KubernetesCluster k8sCluster, Map<String, LiveSystemComponentDto> lsDtoMap) {
    ...
    k8sCluster.getMonitoringInstances().forEach(component -> asserMonitoringComponent(lsDtoMap,
      (CaaSPrometheus) component, provider));
    k8sCluster.getApiGatewayInstances().forEach(component -> assertApiGatewayComponent(lsDtoMap,
      (CaaSAmbassador) component, provider));
    k8sCluster.getK8sWorkloadInstances().forEach(component -> assertK8sWorkloadComponent(lsDtoMap, component,
      provider));
    k8sCluster.getServiceMeshSecurityInstances().forEach(component -> assertSericeMeshSecurityComponent(lsDtoMap,
      component, provider));
    k8sCluster.getLoggingInstances().forEach(component -> assertLoggingComponent(lsDtoMap,
      (CaaSElasticLogging) component, provider));
  }

  private void assertLoggingComponent(
    Map<String, LiveSystemComponentDto> lsDtoMap,
    CaaSElasticLogging component,
  ...
  private void assertSericeMeshSecurityComponent(
    Map<String, LiveSystemComponentDto> lsDtoMap,
    CaaSServiceMeshSecurityImpl component,
    ProviderType provider)
  {
    var dto = lsDtoMap.get(component.getId().getValue());
  ...
  private void assertK8sWorkloadComponent(
    Map<String, LiveSystemComponentDto> lsDtoMap,
    CaaSKubernetesWorkload component,
    ProviderType provider)
  {
    var dto = lsDtoMap.get(component.getId().getValue());
  ...
  private void assertApiGatewayComponent(
    Map<String, LiveSystemComponentDto> lsDtoMap,
    CaaSAmbassador component,
    ProviderType provider)
  {
    var dto = lsDtoMap.get(component.getId().getValue());
  ...
  private void asserMonitoringComponent(
    Map<String, LiveSystemComponentDto> lsDtoMap,
    CaaSPrometheus component,
    ProviderType provider)
  {
    var dto = lsDtoMap.get(component.getId().getValue());
